...
#: repository alone.
#: The product mark, as it appears on every document. A PNG rather than the
#: window's SVG pair: it is one blue on transparency, so it reads on a light
#: and a dark page alike and needs no second file, and a rasterised mark is
#: what the PDF renderer embeds anyway.
_LOGO_PATH = Path(__file__).resolve().parent.parent / "ui" / "design" / "assets" / "logo.png"
...
_LABELS = {
    "en": dict(
        ...
        wcag="WCAG", footer="Generated by XAnalyze.",
        ...
        cat={"ai-text": "AI-sounding text", "typography": "Typography",
    ...
    "uk": dict(
        ...
        wcag="WCAG", footer="Згенеровано XAnalyze.",
        ...
        cat={"ai-text": "Схоже на текст ШІ", "typography": "Типографіка",
    ...
    "it": dict(
        ...
        engine="Rilevato da", wcag="WCAG", footer="Generato da XAnalyze.",
        ...
        cat={"ai-text": "Testo dall'aspetto artificiale", "typography": "Tipografia",
...
#: Which media type the data URI declares, by suffix. Getting this wrong
#: does not raise - it renders a broken image, which is worse.
...
def _logo_data_uri() -> str:
    try:
        ...
        return ""
    media = _LOGO_MEDIA_TYPES.get(_LOGO_PATH.suffix.lower(), "image/png")
...
def render_html(model: ReportModel, lang: str = "en") -> str:
    ...
    # The heading is the name; the target is the line under it. A report
    # headed by `/Users/…/simulations/mixed-problems` wraps to three lines,
    # is mostly somebody's home directory, and says least at exactly the
    # moment a reader is deciding what they are looking at. `display_name`
    # answers "what is this a report about"; `target` stays, verbatim,
    # underneath, where a path is readable rather than a title.
    from report.model import display_name

    name = display_name(model.meta.target, getattr(model.meta, "repo", ""))
    doc_title = f'{mode_label} — {name}' if name else labels["title"]
    # The target line, and the checkout it came from when the address alone
    # cannot say: `127.0.0.1:5173` names a port on one machine.
    target_line = ""
    if model.meta.target and model.meta.target != name:
        target_line = _esc(model.meta.target)
    repo = getattr(model.meta, "repo", "")
    if repo and repo != model.meta.target:
        served = _esc(labels["served_from"].format(repo=_esc(repo)))
        target_line = f"{target_line} &middot; {served}" if target_line else served
...
.report-header {{
  /* Top-aligned, not centred. The text column is four lines now - name,
     target, mode, run parameters - and a centred mark floats halfway down
     it, level with nothing. Against the first line it reads as belonging
     to the name. */
  display: flex; align-items: flex-start; gap: 5mm;
  border-bottom: 1px solid {palette.border}; padding-bottom: 5mm; margin-bottom: 8mm;
}}
/* 14 mm, not 22. The mark is an identifier at the top of a technical
   document, not the subject of the page: at 22 mm it took a fifth of the
   header's width and pushed the name and the target into a narrow column. */
.report-header img {{ width: 14mm; height: auto; flex: 0 0 auto;
  margin-top: 0.5mm; }}
...
/* The path or address, set apart from the name above it: monospace, because
   it is a literal a reader may retype, and quiet, because it is the answer
   to "which one" and not to "what is this". */
.report-header .target {{ margin: 0 0 1mm; font-size: 8.5pt;
...
<header class="report-header">
  {logo_img}
  <div>
    <h1>{_esc(name or labels["title"])}</h1>
